...
大学研究室の流体現象の観測実験において、高速カメラで取得される膨大な画像データから衝撃波が発生しているフレームを特定する作業は、従来手作業で行われており、膨大な解析コストが課題でした。本研究では、**CenterNet** を用いて、全体が写っている衝撃波（a, b）から一部が見切れた衝撃波（c）までを検出するモデルを構築しました。これにより、従来の手作業による解析コストを大幅に削減し、データ解析の自動化を実現しました。
